...
from functions.py3compat import izip, izip_longest, StringIO, cPickle_load, \
...
def argsort(lst):
    ...
    return sorted(list(range(len(lst))), key=lst.__getitem__)
...
def edges_of_cycle(seq):
    ...
    next(b, None)
    return zip_longest(a, b, fillvalue=seq[0])

def to_cycle(simple_path_nodes):
    seq = rotate_min_to_first_pos(simple_path_nodes)
    a, b = itertools.tee(seq)
    next(b, None)
    return tuple( zip_longest(a, b, fillvalue=seq[0]) )
